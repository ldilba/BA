...
Innerhalb eines Channels wird letztendlich die Queue definiert, in der die Nachrichten zwischengespeichert werden. Im folgenden Codeausschnitt ist gezeigt, wie eine Connection, ein Channel und eine Queue erstellt werden.
...
Eine Nachricht kann über \texttt{basic\_{}publish} in einem Channel in eine Queue geschrieben werden. 

\begin{lstlisting}[language=Python]
channel.basic_publish([...], body=f'{{"uid": "{uid}", "service": "{service}", "query": "{query}", "params":{json.dumps(params)}}}'.encode('utf-8'))
\end{lstlisting}
